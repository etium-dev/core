...
cp -r /tmp/etium-src/ai-engineer ai-engineer 2>/dev/null \
  || { git clone -q --depth 1 https://github.com/etium-dev/core /tmp/etium-src && cp -r /tmp/etium-src/ai-engineer ai-engineer; }
grep -qxF '.etium/' .gitignore 2>/dev/null || printf '.etium/\n' >> .gitignore
git add ai-engineer .gitignore
git commit -m "Add etium ai-engineer package"
```

**Verify**: `test -f ai-engineer/loop.ts && test -f ai-engineer/github.ts && ls ai-engineer/templates/*.md | wc -l` →
the file tests exit 0 and the count is `7`.
...
cd "$REPO_DIR" && ETIUM_GH_REPO="$GITHUB_REPO" ETIUM_GH_TRUSTED="$TRUSTED" \
  ETIUM_GH_AGENT="$AGENT_LOGIN" etium tick --surface ai-engineer/github.ts
```
...
```
* * * * * cd REPO_DIR && ETIUM_GH_REPO=… ETIUM_GH_TRUSTED=… ETIUM_GH_AGENT=… etium tick --surface ai-engineer/github.ts >> /tmp/etium-tick.log 2>&1
```

If `INSTALL_CRON=yes`: install it idempotently —

```sh
( crontab -l 2>/dev/null | grep -v 'etium tick --surface ai-engineer/github.ts'; \
  echo '* * * * * cd '"$REPO_DIR"' && ETIUM_GH_REPO='"$GITHUB_REPO"' ETIUM_GH_TRUSTED='"$TRUSTED"' ETIUM_GH_AGENT='"$AGENT_LOGIN"' etium tick --surface ai-engineer/github.ts >> /tmp/etium-tick.log 2>&1' ) | crontab -
crontab -l | grep -c 'etium tick --surface ai-engineer/github.ts'
```
