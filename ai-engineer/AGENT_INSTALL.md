...
```sh
cd "$REPO_DIR" && ETIUM_GH_REPO="$GITHUB_REPO" ETIUM_GH_LOOP="$LIBRARY/loop.ts" \
  etium tick --surface github
```
...
3. Remaining manual steps, chosen from: install the GitHub CLI; create
   ...
   --with-token --insecure-storage`, pasting a classic token with the
   `repo`, `read:org`, and `gist` scopes (gh's own required minimum) for
   the account the engineer should act as;
   ...
   assign the deployment's account to a GitHub issue to start the first
   attempt.
