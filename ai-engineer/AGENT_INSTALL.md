...
3. **Wire it to GitHub, so assigning an issue starts work and `/et`
   ...
   - "The engineer wakes up on a schedule. While you're trying things
     out, the simplest option is running `etium watch` in a terminal
     whenever you're using it — nothing gets installed. For always-on,
     it's a once-a-minute cron entry: should I add that to your crontab
     now, leave the exact line in my report for you, or will you just use
     `etium watch` for now?" → `INSTALL_CRON` (`yes` / anything else =
     print; the watch choice also just prints the line for later)
