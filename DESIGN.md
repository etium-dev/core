...
| `run.interrupted` | `tick` | stale lock info (pid, host, age) |
...
- **`Etium tick`**: idempotent janitor. For each run: detect dead supervisors (lock pid not alive, or foreign-host lock with stale mtime) → append `run.interrupted`, clear lock; if the run is resumable (interrupted, or parked with a pending decision in the mailbox) → spawn a detached supervisor. Guarded by a global tick lock so overlapping cron invocations no-op. In M2, `tick` also drives pull-based surface adapters (§10.3).
...
Per-step: `wall` (always enforceable), `tokens`, `cost` (enforceable only when the adapter reports usage; otherwise the step is marked **unmetered** in `step.started`), `stallWarn` (default 15m — emits `budget.warning`, never kills; long tool or model work is legitimate). Per-run: `wall`, `cost`, and a step-count guard (default 100). Iteration limits are loop logic, not core.
...
API surface (complete): `run.step(name, opts)`, `run.gate(name, opts?)`, `run.effect(name, fn)`, `run.params`, `run.abandon(reason)`, `t(file)`. That is the whole vocabulary. Reference loops shipped: `ralph`, `plan-implement` (above), `triage`. Everything else is user-authored.
...
- **Env profiles**: named environment sets defined in config, e.g. `agent` (minimal, credential-free) and `publish` (holds `GH_TOKEN` etc.). Every step names its profile; default is `agent`. Publication (push, PR creation) happens in `exec` steps under `publish`. This is Invariant 8 made concrete; it preserves the predecessor system's hard-won boundary.
- **Redaction**: the step runner scans raw stream lines for exact values of profile variables marked secret and writes `[redacted:VAR]` before anything touches disk. Documented limitation: encoded or split secrets are not caught; raw contains repository content by nature.
...
interface HarnessAdapter {
  ...
  parse(line: string): HarnessEvent[] | null;   // null = unrecognized line (kept in raw only)
}
...
`grade: string | { command: string }` on a step: after the harness exits, the runner executes the command in the workspace; exit 0 ⇒ `passed`, stdout captured as an artifact. A grader that needs a model is just another step with a different harness/model — the maker/checker split is loop composition, not a core feature.
...
Etium/
  packages/
    core/        # ≤ ~3,000 LOC: ledger, fold, engine, runner, budgets, lock, mailbox, tick
    cli/
    adapters/    # exec, replay, codex, pi, claude, openhands — each ≤ ~300 LOC
  loops/         # ralph, plan-implement, triage — each ≤ ~150 LOC
  fixtures/      # captured raw streams per harness (golden transcripts)
  schema/        # JSON Schema for envelope + events, versioned
  docs/
```
...
3. Run-ID format and collision handling — trivial, settle at M0.
4. npm name: `Etium` is unclaimed as of 2026-07-30. GitHub org availability unverified (API rate limit). Claim both before M0 publicity.
5. License: MIT proposed, matching Pi and OpenHands.
...
| Controller/agent credential boundary | Env profiles (`agent` vs `publish`) |
